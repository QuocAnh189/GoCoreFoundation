...
import (
	...
	jwtutil "github.com/QuocAnh189/GoCoreFoundation/root/jwt"
	"github.com/QuocAnh189/GoCoreFoundation/root/session"
	"github.com/QuocAnh189/GoCoreFoundation/root/util"
...
func (j *JwtSessionProvider) GetSessionFromRequest(r *http.Request) (*SessionResult, error) {
	...
	if isExpired || err != nil {
		didAutoRefresh = true
		fmt.Printf(">> JwtSessionProvider: session expired, auto-refreshing...")
...
func (j *JwtSessionProvider) getOrCreateJwtToken(r *http.Request) (*JwtResult, error) {
	...
	if err == ErrMalformedJwt {
		fmt.Printf(">> JwtSessionProvider: WARNING ignoring your jwt token - totally malformed JWT token")
	} else if jwtResult == nil || jwtResult.JwtToken == nil || jwtResult.SessionKey == "" || jwtResult.AuthToken == "" || err != nil {
		fmt.Printf(">> JwtSessionProvider: WARNING ignoring your jwt token - error getting JWT from request:", err)
	} else {
		fmt.Printf(">> JwtSessionProvider: jwt ok")
	}
...
func (j *JwtSessionProvider) getAuthTokenFromJwtToken(jwtToken *jwt.Token) (string, error) {
	...
	if authToken == "" || err != nil {
		fmt.Printf(">> JwtSessionProvider: error signing JWT:", err)
		return "", err
	}
