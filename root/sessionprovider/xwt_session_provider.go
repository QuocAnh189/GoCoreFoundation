...
func (x *XwtSessionProvider) GetSessionFromRequest(r *http.Request) (*SessionResult, error) {
	...
	if sess == nil || !ok {
		sess, err = x.initNewSession(sessionKey, authToken, "root.xwt_session_provider")
		if err != nil {
	...
	if isExpired || err != nil {
		didAutoRefresh = true
		fmt.Printf(">> XwtSessionProvider: session expired, auto-refreshing...")
...
func (x *XwtSessionProvider) getOrCreateXwtToken(r *http.Request) (*XwtResult, error) {
	...
	if xwtResult == nil || xwtResult.XwtToken == "" || xwtResult.SessionKey == "" || err != nil {
		fmt.Printf(">> XwtSessionProvider: WARNING ignoring your xwt token - error getting XWT from request:", err)
	} else {
		fmt.Printf(">> XwtSessionProvider: xwt ok")
	}
...
func (x *XwtSessionProvider) createNewXwtToken() (*XwtResult, error) {
	...
	claims := jwt.NewClaims(sessionKey)
